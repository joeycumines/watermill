...
func TestFanOut(t *testing.T) {
	...
	upstreamPubSub := memchan.NewGoChannel(memchan.Config{}, logger)

	fanout, err := memchan.NewFanOut(upstreamPubSub, logger)
...
func TestFanOut_RouterClosed(t *testing.T) {
	logger := watermill.NopLogger{}
	pubSub := memchan.NewGoChannel(memchan.Config{}, logger)

	fanout, err := memchan.NewFanOut(pubSub, logger)
	require.NoError(t, err)

...
func TestPublishSubscribe_not_persistent(t *testing.T) {
	messagesCount := 100
	pubSub := memchan.NewGoChannel(
		memchan.Config{OutputChannelBuffer: int64(messagesCount)},
		watermill.NewStdLogger(true, true),
	)
...
func TestPublishSubscribe_not_persistent_with_context(t *testing.T) {
	messagesCount := 100
	pubSub := memchan.NewGoChannel(
		memchan.Config{OutputChannelBuffer: int64(messagesCount), PreserveContext: true},
		watermill.NewStdLogger(true, true),
	)
	topicName := "test_topic_" + watermill.NewUUID()
